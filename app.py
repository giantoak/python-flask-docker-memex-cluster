...
def analyze_clusters():
    ...
    geopts = map(lambda x: (float(x['_source']['latitude']), float(x['_source']['longitude'])),
                 geo)
    ethnicity = filter(lambda x: 'ethnicity' in x['_source'].keys(), res['hits']['hits'])
    ethnicity = map(lambda x: str(x['_source']['ethnicity']),
                    ethnicity)
    city = filter(lambda x: 'city' in x['_source'].keys(), res['hits']['hits'])
    city = map(lambda x: str(x['_source']['city'].encode('ascii', 'ignore')),
               city)
    ...
    time_dist = map(lambda x: (x['_source']['latitude'],
                               x['_source']['longitude'],
                               datetime.strptime(x['_source']['posttime'], "%Y-%m-%dT%H:%M:%S").date()),
                    time)

    imps = list()  # implied travel speed
    imps2 = list()  # average distance between multiple posts at exact timestamp
    total_ts = sorted(time_dist, key=lambda x: x[2])

    for item in window(total_ts, 2):
        dist = vincenty((item[0][0], item[0][1]), (item[1][0], item[1][1])).miles
        time = abs(item[1][2] - item[0][2]).total_seconds()/(24*3600.)
    ...
    total_time = abs(total_ts[-1][2] - total_ts[0][2]).total_seconds()/(24*3600.)
    ...
    return json.dumps({'avg_price_quantile': np.mean(pq),
                       'loc': location,
                       'ethnicity': eth,
                       'price_var': np.std(raw),
                       'mean_price': np.mean(raw),
                       'implied_travel_time': np.mean(imps),
                       'total_time': total_time,
                       'max_dist_sim_posts': np.max(imps2),
                       'no_cites': len(cities),
                       'avg_post_week': avg_post_week})
